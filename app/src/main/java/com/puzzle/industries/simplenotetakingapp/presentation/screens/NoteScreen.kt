package com.puzzle.industries.simplenotetakingapp.presentation.screens
...
import com.puzzle.industries.simplenotetakingapp.presentation.components.BackPressHandler
import com.puzzle.industries.simplenotetakingapp.presentation.components.OutlinedTextInput
import com.puzzle.industries.simplenotetakingapp.presentation.components.topHeader
import com.puzzle.industries.simplenotetakingapp.presentation.previewProviders.providers.NoteScreenDataProvider
...
fun NoteScreen(
    ...
    val note: NoteDto? = if(noteId.isNullOrBlank()) null else noteViewModel.getNoteById(UUID.fromString(noteId))
    ...
    val attemptToSaveNoteAndExit: () -> Unit = {
        ...
        if (newNote.title.isNotBlank() || newNote.description.isNotBlank()) {
            if (note == null) {
                noteViewModel.addNote(newNote)
            } else {
                newNote.id = note.id
                newNote.lastModifyDate = note.lastModifyDate
                if (note != newNote) {
                    newNote.lastModifyDate = Date()
